...
function create_2d_array(height, width, default_value)
{
	var board = [];
...
function set_position(dom, left, top)
{
	dom.css('left', left+'px');
	dom.css('top', top+'px');
}

function set_css(dom, css)
{
	for (var key in css)
	{
		dom.css(key, css[key]);
...
function shake(args)
{
	...
	if(is_ndef(temp))
	{
		temp = {attr: ani_attr, value: target.css(ani_attr), count: 1};
		shake_temp[target] = temp;
	}
	else
	{
		shake_temp[target].count++;
	}
	var ani0 = {};
